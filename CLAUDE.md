...
- Routing is React Router; [src/App.tsx](frontend/src/App.tsx) holds the main feed and most local form state (`useState`). Other routes: `ProfilePage` (`/users/:id` — includes a self-only "Your posts" list), `SettingsPage` (`/settings` — profile, password, theme), `AdminPage` (`/admin` — stats, moderation queue, user management + impersonation; redirects non-staff; the Header's navigation lives in a user dropdown — Profile / Admin (staff only) / Settings / Log out — next to the notification bell, and the navbar wordmark uses the same `--gradient-brand` text treatment as the footer). Static info pages (`/about`, `/help`, `/privacy`, `/terms`, `/faq` — the FAQ uses native `<details>` toggles) are routed in both auth branches, so they work logged-out (brand-only navbar) and logged-in (full Header); the footer and the user dropdown link to them. Impersonation keeps the original staff session in the auth store (`impersonator`, persisted to localStorage) and shows a fixed amber `ImpersonationBanner` above the navbar (`body.impersonating` shifts the layout) with a Stop button that restores the staff session and clears the React Query cache.
