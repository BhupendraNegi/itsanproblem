...
- **Styling is a hand-rolled CSS design system in [src/App.css](frontend/src/App.css)** (not Tailwind, despite older docs) — design tokens as CSS custom properties on `:root`, referenced by component class names. The palette is **Instagram-style**: an Instagram blue (`--primary`) for buttons, a pink (`--accent`) for likes/active states, and the signature multicolor gradient (`--gradient-brand`) on the nav bar and avatars. **Dark mode is automatic** via `@media (prefers-color-scheme: dark)`, which overrides the semantic token values (and inverts content-area line icons) — there is no toggle; it follows the browser/OS setting. When styling, add/extend semantic tokens rather than hardcoding colors so both themes stay correct. Global chrome: [Header.tsx](frontend/src/components/Header.tsx) (nav) and [Footer.tsx](frontend/src/components/Footer.tsx) (rendered once in [App.tsx](frontend/src/App.tsx)). See [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md).
